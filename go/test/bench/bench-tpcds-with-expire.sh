...
start_streamer() {
  ...
  /tmp/janitor-streamer-b > "$out_log" 2>&1 &
  echo $!
}
...
EOF
  ...
  set +e
  files_ss=$(JANITOR_WAREHOUSE_URL="$warehouse_url" S3_ENDPOINT="${S3_ENDPOINT:-}" S3_REGION="${S3_REGION:-${AWS_REGION:-us-east-1}}" /tmp/janitor-cli-b analyze "${NAMESPACE}.db/store_sales" 2>/dev/null | awk '/^Data files:/{print $3}')
  files_sr=$(JANITOR_WAREHOUSE_URL="$warehouse_url" S3_ENDPOINT="${S3_ENDPOINT:-}" S3_REGION="${S3_REGION:-${AWS_REGION:-us-east-1}}" /tmp/janitor-cli-b analyze "${NAMESPACE}.db/store_returns" 2>/dev/null | awk '/^Data files:/{print $3}')
  files_cs=$(JANITOR_WAREHOUSE_URL="$warehouse_url" S3_ENDPOINT="${S3_ENDPOINT:-}" S3_REGION="${S3_REGION:-${AWS_REGION:-us-east-1}}" /tmp/janitor-cli-b analyze "${NAMESPACE}.db/catalog_sales" 2>/dev/null | awk '/^Data files:/{print $3}')
  set -e
